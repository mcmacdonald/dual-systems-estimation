...
* The contents of this folder contains both R ('demo.R') and Python ('demo.py') scripts to estimate the probability that an incident is known to the police. To do so, it uses 2019 police-reported crime incidents reported by Statistics Canada and self-reported information about victimization from the 2019 General Social Survey on Victimization. The scripts estimate and visualize the probability distributions for different types of crime, adjusting for attributes of the respondents, offenders, and incidents that make reporting/non-reporting to the police more or less likely.
